...
\paragraph{Appendix:} Contains the GitHub link and installation instructions.
...
The project aims to develop a web application allowing users to create and manage their custom game event. The core objective of this project is to try to eliminate the inefficiency of managing player-made games through the use of matchmaking. For the system to be practical, We aim to implement various features to enable users to manage their games effectively.
...
The project aims to be accessible and easy to understand to ensure a smooth experience for the user. The implementation and design of the system will be discussed in detail below.
...
\begin{figure}[H]	
	\centering
	\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{img/FinalMatch.png}
	\caption{Final Match}
	\label{tikz:finalmatch}
\end{figure}

When the number of participants reaches the amount needed to start a game, matchmaking starts instantly, and teams are formed as seen in figure \ref{tikz:finalmatch}. \newline
The ability to validate who won the match is only visible to the creator of the match (Admin).
\newpage
...
The $rank$ and $tier$ is the players rating, I normalised this rating into its integer equivalent, by comparing it to a table I created of constant values.
...
We then calculate the average mmr (matchmaking rating) of all participants (summoners)
...
Using combinations in the itertools library \cite{bernard2016iterators}, we can evaluate the number of possible teams.
We setup an initial high value $b = 10000$ so we can figure out which team in possible teams to deal with first. For each team in possible teams, we can determine the best team using:

\begin{minted}{python}
for team in possible_teams:
 if abs(team.mmr - avg_mmr) < b:
 	team1 = team
	b = abs(team.mmr - avg_mmr)
possible_teams.remove(team1)
\end{minted}

This step is repeated for team two, but we skip any teams where players overlap.
\begin{minted}{python}
...
Although this method is slower then just putting every other player on team 2 based on a sorted list. The method stated above presents more accurate results, meaning more balanced teams.  
...
The functionality of accessing API's was kept generalised as possible to future proof it allowing more game platforms to be added.\hfill \break 
\newpage
...
Here is an example of a selenium test case in figure \ref{tikz:seleniumtc} which simulates the login functionality.
\begin{table}[h]
	\centering
	\begin{tabular}{ll}
		\toprule
...
Testing the back-end and its containing functionality, we use postman because requests that were made also tested functionality of the matchmaking and point system.

\chapter{System Evaluation}
\section{Robustness}
To ensure the system was robust I conducted multiple types of tests, including:
...
Theses tests were responsible for testing various components of the project; unit tests were integrated to test individual components, we try to emulate how a real-world user would use the system through automated tests.\newline
Incorporating multiple tests allows for the more natural discovery of errors that may have been overlooked. 
When using testing techniques like e2e testing and automated browser testing, we can visually see through the web browser what elements were being tested. Testing ensured we did not neglect any part of the system and guaranteed the application is robust.
Using different testing techniques like unit testing and automated testing meant that we could examine all areas of the application, assuring that everything is operating as designed. 
\section{Limitations}
...
Install Angular CLI (Note: Node may have to be installed [\href{https://nodejs.org/en/}{Node.js}])
\begin{minted}{bash}
